 ...
 */
require_once strtr(realpath(dirname(__FILE__)) . '/DiscussTestCase.php','\\','/');
/**
 * Main Discuss test harness.
 *
 * @package discuss-test
 */
class DiscussTestHarness {
    /**
     ...
     *
     * @return DiscussTestHarness
     */
    public static function suite() {
        $suite = new DiscussTestHarness('DiscussHarness');
        return $suite;
    }
    ...
    public static function _getConnection($options = array()) {
        $modx = DiscussTestHarness::$modx;
        if (is_object($modx)) {
            if (!$modx->request) { $modx->getRequest(); }
            if (!$modx->error) { $modx->request->loadErrorHandler(); }
            $modx->error->reset();
            DiscussTestHarness::$modx = $modx;
            return DiscussTestHarness::$modx;
        }
