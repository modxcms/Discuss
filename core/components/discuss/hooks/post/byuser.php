...
$postTpl = $modx->getOption('postTpl',$scriptProperties,'post/disUserPostLi');
$user = (int)$modx->getOption('user', $scriptProperties, 0);

$cacheKey = 'discuss/byuser/'.$user.'.'.md5(serialize($scriptProperties));
$cache = $modx->cacheManager->get($cacheKey);
if (!empty($cache)) {
    /* Check to make sure there's no new posts by this user since we cached */
    if (isset($cache['cachedon'])) {
        $c = $modx->newQuery('disPost');
        $c->where(array(
            'author' => $user,
            'AND:createdon:>' => strftime(Discuss::DATETIME_FORMATTED, $cache['cachedon'])
        ));
